...
const Header: React.FC<HeaderProps> = ({ btc, isLive, onToggleLive, aiStatus, isPaid, networkStatus }) => {
  ...
  return (
    <header className="flex flex-col md:flex-row items-center justify-between gap-4 py-4 px-6 glass rounded-2xl border-b border-amber-500/20 shadow-2xl relative overflow-hidden">
      <div className="flex items-center gap-4">
        <div className={`w-12 h-12 flex items-center justify-center rounded-2xl shadow-lg transition-all ${isPaid ? 'bg-amber-500 shadow-amber-500/30' : 'bg-teal-500 shadow-teal-500/30'}`}>
        ...
        <div>
          <h1 className="text-2xl font-black tracking-tighter text-white uppercase leading-none">ProfitMax <span className={isPaid ? "text-amber-500" : "text-teal-400"}>PM5</span></h1>
          <div className="flex items-center gap-2 mt-1">
            <span className="text-[10px] text-slate-500 font-bold uppercase mono tracking-widest">{isPaid ? 'ELITE_PRO_ULTRA' : 'BASIC_SIM'}</span>
      ...
      <div className="flex gap-8 items-center bg-slate-900/40 p-3 px-6 rounded-2xl border border-white/5">
        <div className="flex flex-col min-w-[140px]">
          <div className="flex items-center gap-1">
             <span className="text-[10px] font-bold text-slate-500 uppercase tracking-widest">BTC/USD LIVE</span>
             <span className="text-[8px] bg-amber-500 text-black px-1 rounded font-black">PRO</span>
          </div>
        ...
        <div className="hidden lg:flex flex-col">
          <span className="text-[10px] font-bold text-slate-500 uppercase tracking-widest">VOL_PRESSURE</span>
          <div className="flex items-center gap-1 mt-1">
            {Array.from({ length: 12 }).map((_, i) => (
              <div 
                key={i} 
                className={`w-1 h-4 rounded-full transition-all duration-300 ${i < btc.momentum_score * 1.2 ? (i > 9 ? 'bg-rose-500' : 'bg-amber-500') : 'bg-slate-800'}`}
              ></div>
            ))}
          </div>
        </div>
        ...
        <div className="flex flex-col items-end">
          <span className="text-[10px] font-bold text-slate-500 uppercase tracking-widest mb-1">TERMINAL_STATE</span>
          <button 
