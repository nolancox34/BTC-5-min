...
const App: React.FC = () => {
  ...
  const [insight, setInsight] = useState<string>("Initializing Pro-Tier Oversight...");
  ...
  useEffect(() => {
    ...
    const interval = setInterval(runTick, 4000); // Faster tick for pro feel
  ...
  useEffect(() => {
    ...
    fetchInsight();
    const heartbeat = setInterval(fetchInsight, 25000);

    const volCheck = setInterval(() => {
        const move = Math.abs(state.btc.price - lastBtcPrice.current) / (lastBtcPrice.current || 1);
        if (move > 0.0015) fetchInsight();
  ...
  return (
    <div className="min-h-screen relative overflow-hidden flex flex-col p-4 md:p-6 lg:p-8">
      ...
      <div className="relative z-10 max-w-7xl mx-auto w-full flex flex-col gap-6 h-full">
        ...
        <div className="glass p-4 rounded-xl border-l-4 border-l-amber-500 bg-amber-950/10 transition-all shadow-xl shadow-amber-950/20">
          <div className="flex items-center justify-between">
            <div className="flex items-center gap-3">
              ...
              <div>
                <span className="text-[10px] font-bold text-amber-500 uppercase tracking-widest block mb-0.5">
                  {aiStatus === 'thinking' ? 'ANALYZING MARKET VECTORS...' : 'GEMINI PRO STRATEGIC OVERLAY'}
                </span>
            ...
            {!hasPaidKey && (
              <button 
                ...
                className="text-[10px] font-bold bg-amber-500 text-black px-4 py-1.5 rounded-lg hover:bg-amber-400 transition-all shadow-lg shadow-amber-500/20 active:scale-95"
              >
                LINK PAID ACCOUNT
              </button>
            )}
        ...
        <div className="grid grid-cols-1 lg:grid-cols-12 gap-6 h-full min-h-[600px]">
          <div className="lg:col-span-8 space-y-6">
            <div className="flex justify-between items-end">
              <h2 className="text-xl font-bold tracking-tight text-white flex items-center gap-2">
                ACTIVE MARKETS <span className="bg-amber-500 h-1.5 w-1.5 rounded-full animate-ping"></span>
              </h2>
              <div className="flex gap-4 text-[10px] font-bold text-slate-500 uppercase">
                <span>Network: <span className={MarketSimulator.networkStatus === 'stable' ? 'text-emerald-400' : 'text-amber-400'}>{MarketSimulator.networkStatus.toUpperCase()}</span></span>
            ...
            <div className="grid grid-cols-2 md:grid-cols-4 gap-4 mt-6">
              {[
                ...
                { label: 'Execution Speed', value: 'INSTANT', color: 'text-sky-400' },
                ...
                <div key={i} className="glass p-3 rounded-xl border border-slate-800 text-center hover:bg-white/5 transition-colors group">
                  <div className="text-[10px] text-slate-500 font-bold uppercase mb-1 group-hover:text-slate-400">{stat.label}</div>
          ...
          <div className="lg:col-span-4 flex flex-col gap-6">
             ...
             <div className="glass p-6 rounded-xl border border-amber-500/20 shadow-inner">
               <div className="flex justify-between items-center mb-4">
                 <h3 className="text-xs font-bold text-amber-500 uppercase tracking-widest">Active PnL Account</h3>
                 <div className="text-[10px] text-amber-300/30">SIMULATED EXECUTION</div>
               </div>
               <div className="space-y-4">
                  ...
                  <div className="flex justify-between items-end border-t border-slate-800 pt-4">
                    ...
                    <span className="text-lg font-bold mono text-amber-400">{state.wallet.active_positions} SNIPES</span>
                  </div>
                  <div className="bg-amber-500/5 p-4 rounded-xl border border-amber-500/10">
                    <div className="text-[10px] text-amber-500 uppercase mb-1 font-bold">Today's Realized Returns</div>
